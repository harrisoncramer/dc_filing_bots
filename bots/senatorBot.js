...
const bot = async (page, today) => {
    ...
    .then(async({ newData, updates}) => composeEmail({ newData, updates, collection: Senator }))
    .then(async({newData, updates}) => {
        if(newData.length > 0 || updates.length > 0){
            const emails = await getUsers({ "data.senators": true });
            return mailer({ emails, subject: 'Senate Disclosure(s)', mailDuringDevelopment: false, date: today, bot: 'senatorBot' }).then((res) => {
